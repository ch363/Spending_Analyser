"""Cash-flow projection helpers powering PlainSpend analytics."""
...
def compute_cashflow_runway(
    ...
    ----------
    expenses:
        Expense transactions containing a ``spend`` column (positive values).
    incomes:
        Income transactions with ``amount`` and ``date`` columns.
    today:
        Current date used as the anchor for the projection.
    total_spend_to_date:
        Sum of spend already incurred this month.
    projected_total:
        Forecast total spend by month end.
    days_remaining:
        Number of days left in the current month.
    recurring_entries:
        Iterable of recurring commitments to include in the runway.

    Returns
    -------
    CashflowRunway
        Structure containing daily safety, commitment and budget insights.
    """
